...
<script lang="ts">
  ...
  interface Props {
    ...
    columns?: 2 | 3;
    /** on a plum/violet section: translucent panels + light text. */
    dark?: boolean;
  }
  let { items, columns = 3, dark = false }: Props = $props();
  ...
  {#each items as item (item.title)}
    ...
    <li class="h-full">
      ...
      >
        {#if item.icon}
          <span
            class="brand-gradient tilt-neg mb-5 inline-flex h-12 w-12 items-center justify-center rounded-xl text-white shadow-sm"
          >
            <Icon name={item.icon} size={24} />
          </span>
        {/if}
        <h3 class="t-h3 {dark ? 'text-white' : 'text-ink group-hover:text-violet-700'}">{item.title}</h3>
        <div class="mt-2">
          <Prose md={item.body} class="!text-[16.5px] !leading-relaxed {dark ? 'on-dark' : ''}" />
        </div>
        {#if href}
          <span
